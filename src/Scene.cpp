...
Scene::Scene() {
    ...
    this->drone[0] = Drone("../data/drone0_droneBody.txt", "../data/drone0_rotor0.txt",
                           "../data/drone0_rotor1.txt", "../data/drone0_rotor2.txt",
                           "../data/drone0_rotor3.txt", initialPosDrone0, initialOrientation);

    this->drone[1] = Drone("../data/drone1_droneBody.txt", "../data/drone1_rotor0.txt",
...
Scene::Scene(double _XRange[2], double _YRange[2], double _ZRange[2]) {
    ...
    for(int h = 0; h < NUMBER_OF_DRONES; ++h){
        ...
        for( int i = 0; i < NUMBER_OF_ROTORS; ++i){
            GNU.DodajNazwePliku(this->drone[h][i].getSolidFileName().c_str())
                    .ZmienSposobRys(PzG::SR_Ciagly)
                    .ZmienSzerokosc(1)
                    .ZmienKolor(CHOSEN_DRONE_COLOR);
        }
        /* drone body file */
        GNU.DodajNazwePliku(this->drone[h].getdroneBody().getSolidFileName().c_str())
                .ZmienSposobRys(PzG::SR_Ciagly)
                .ZmienSzerokosc(1)
                .ZmienKolor(CHOSEN_DRONE_COLOR);
    }
...
/* Drawing drones*/
    ...
    for(int f = 0; f < NUMBER_OF_DRONES; ++f){
        /* Draw body of a drone[f]*/
        os.open(this->drone[f].getdroneBody().getSolidFileName());
        if(!os) {
        ...
        for(int h = 0; h < NUMBER_OF_ROTORS; ++h){
            os.open(this->drone[f][h].getSolidFileName());
            if(!os) {
...
void Scene::animateDroneTranslation(double angleOfFlight, double lengthOfFlight) {
    ...
    while(animateVector.getLength() < targetPosFromDroneCenter.getLength()){
        ...
        /* keep in mind that hexes after rotating for 60 degrees looks the same */
...
void Scene::changeDronesColors() {
    if(this->chosenIndex == 0){
        for(int i = 0; i < NUMBER_OF_ROTORS; ++ i){
            GNU.DodajNazwePliku(this->drone[0][i].getSolidFileName().c_str())
                    .ZmienSposobRys(PzG::SR_Ciagly)
                    .ZmienSzerokosc(1)
                    .ZmienKolor(CHOSEN_DRONE_COLOR);
        }
        GNU.DodajNazwePliku(this->drone[0].getdroneBody().getSolidFileName().c_str())
                .ZmienSposobRys(PzG::SR_Ciagly)
                .ZmienSzerokosc(1)
                .ZmienKolor(CHOSEN_DRONE_COLOR);

        for(int i = 0; i < NUMBER_OF_ROTORS; ++ i){
            GNU.DodajNazwePliku(this->drone[1][i].getSolidFileName().c_str())
                    .ZmienSposobRys(PzG::SR_Ciagly)
                    .ZmienSzerokosc(1)
                    .ZmienKolor(NOT_CHOSEN_DRONE_COLOR);
        }
        GNU.DodajNazwePliku(this->drone[1].getdroneBody().getSolidFileName().c_str())
                .ZmienSposobRys(PzG::SR_Ciagly)
                .ZmienSzerokosc(1)
                .ZmienKolor(NOT_CHOSEN_DRONE_COLOR);
